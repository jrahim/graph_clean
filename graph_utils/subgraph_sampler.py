...
class SubgraphGenerator(object):
    ...
    def generationCoverEdge(self, minCoverRatio=0.1):
        subgraphs = self.generationCoverNodeByRatio(minCoverRatio=minCoverRatio)
        newSubgraphs = []
        for subgraph in subgraphs:
            newSubgraph = self.addNodeIntoSubgraphUntillMaxNode(subgraph)
        ...
        for i in range(self.numOfNode):
            for j in range(i+1, self.numOfNode):
                ...
                if self.inoutMat[u,v] == 1:
                    if self.nodeReMapping is not None:
                        u, v = self.nodeReMapping[u], self.nodeReMapping[v]
                    if u > v:
                        u,v = v,u
                    all_edges.append((u,v))
        for subgraph in subgraphs:
            for edge in subgraph:
                picked_edges.add(edge)
        
        while True:
            unpicked_edge = set()
            for edge in all_edges:
                if edge not in picked_edges:
                    unpicked_edge.add(edge)
            if len(unpicked_edge) == 0:
                break
            unpicked_edge = list(unpicked_edge)
            new_gen = SubgraphGenerator.fromEdgelist(unpicked_edge,maxNodeNum=self.maxNodeNum,minNodeNum=self.minNodeNum,
            maxCoverRatio=self.maxCoverRatio,verbose=self.verbose)
            newSamplingSubgraphs = new_gen.generationCoverNodeByRatio(minCoverRatio=minCoverRatio)
            newSubgraphs = []
            for subgraph in newSamplingSubgraphs:
                newSubgraph = self.addNodeIntoSubgraphUntillMaxNode(subgraph=subgraph)
                newSubgraphs.append(newSubgraph)
            for subgraph in newSubgraphs:
                subgraphs.append(subgraph)
                for edge in subgraph:
                    u, v = edge
                    if u > v:
                        u, v = v, u
                    picked_edges.add((u,v))
        return subgraphs
    ...
    def addNodeIntoSubgraphUntillMaxNode(self, subgraph):
        ...
        for u in range(self.numOfNode):
            ...
            for v in range(u+1, self.numOfNode):
                ...
                vv = v
                if self.nodeReMapping is not None:
                    uu = self.nodeMapping[u]
                    vv =self.nodeMapping[v]
                if uu > vv:
                    uu,vv = vv,uu
